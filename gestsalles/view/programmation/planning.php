...
?>
		...
		<div id="navbar-buttons">
			<button type="button" title="Valider les modifications"
					onclick="window.history.back();">
				<img src="../../images/buttons/ok.png" alt="Valider" />
			</button>
			<button type="button" title="Revenir à la page précédente"
					onclick="window.history.back();">
				<img src="../../images/buttons/return.png" alt="Retour" />
			</button>
			<button type="button" title="Quitter l'application"
					onclick="window.history.back();" />
...
<?php
	if (isSet($cinema) and isSet($day_timestamp))
	{
		...
		if ($nb_tables > 0)
		{
			...
			{
				$start_val_index = grid($nb_rooms_per_table, 0, $rooms,
						$start_val_index, "");
			}
			grid($nb_rooms_last_table, ($nb_rooms_per_table - $nb_rooms_last_table),
					$rooms, $start_val_index, "");
		}
		else
		{
			grid($nb_rooms_last_table, ($nb_rooms_per_table - $nb_rooms_last_table),
...
?>
	...
	<div id="movie-session-form-div" name="movieSessionFormDiv">
		<form method="post" action="submitForm()" name="movieSessionForm"
				id="movie-session-form">
			...
			<p style="font-size: 13px; font-style: italic; margin-top: 2px; padding-left: 41px;">
				...
				<input style="margin-left: 17px;" type="button" id="remove-button"
						name="removeButton" value="Supprimer" onclick="" />
			</p>
			<p style="margin-top: 0px;"><b>Date :</b> <span id="sessionFormDate"
					style="font-size: 14px;"><?php echo $dateStr ?></span></p>

			<p><b>Horaire :</b>
				<input name="beginHour" id="begin-hour" value="18" type="text"
						class="time-slot"> :
				<input name="beginMin" id="begin-min" value="00" type="text"
						size="2" class="time-slot" /> –
				<input name="endHour" id="end-hour" value="19" type="text"
						class="time-slot"> :
				<input name="endMin" id="end-min" value="30" type="text"
						size="2" class="time-slot" />
			</p>
			...
			<select id="movie-selecter" name="movieSelecter"
			...
			<select id="room-selecter" name="roomSelecter" disabled="disabled"
					style="width: 200px;">
			...
			<div style="text-align: center;">
				<input type="button" name="saveButton" value="Enregistrer"
						onclick="sessionFormSubmit()" />
				<input type="button" name="resetButton" value="Effacer"
						onclick="sessionFormReset()"/>
				<input type="button" name="abortButton" value="Annuler"
						onclick="sessionFormAbort()"/><br/>
			</div>
		</form>
	</div>
